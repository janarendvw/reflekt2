...
async function page({ params }: { params: { reflection: string } }) {
  ...
  const resolvedActionPoints = reflection?.actionPoints.filter(ap => ap.resolved)
  const unresolvedActionPoints = reflection?.actionPoints.filter(ap => !ap.resolved)
  ...
  return (
    <div className="mx-auto flex w-full max-w-screen-md flex-col">
      ...
      <p className="mb-8 font-mono text-sm text-gray-500">
        {reflection?.createdAt.toLocaleDateString()} - {reflection?.reflectionType} model
      </p>
      ...
      <div className="flex flex-col gap-2">
        {unresolvedActionPoints?.map((ap, index) => <ActionPointCard key={ap.title} actionPoint={ap} index={index} />)}
      </div>
      {unresolvedActionPoints?.length === 0 && (
        <Alert className="border-dashed">
          ...
          <p className="text-sm text-muted-foreground">You have no unresolved action points. Keep up the good work!</p>
        </Alert>
      )}
      <h2 className="mb-2 mt-14 font-semibold">Resolved action points</h2>
      <div className="flex flex-col gap-2">
        {resolvedActionPoints?.map((ap, index) => (
          <ActionPointCard resolved key={ap.title} actionPoint={ap} index={index} resolution={ap.resolution} />
        ))}
      </div>
      {resolvedActionPoints?.length === 0 && (
        <Alert className="border-dashed">
          ...
          <p className="text-sm text-muted-foreground">You have no resolved action points. Keep up the good work!</p>
        </Alert>
      )}
    </div>
  )
